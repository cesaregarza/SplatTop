from fast_api_app.routes.admin_tokens import router as admin_tokens_router
from fast_api_app.routes.comp_auth import router as comp_auth_router
from fast_api_app.routes.front_page import router as front_page_router
from fast_api_app.routes.infer import router as infer_router
from fast_api_app.routes.jwks import router as jwks_router
from fast_api_app.routes.ping import router as ping_router
from fast_api_app.routes.player_detail import router as player_detail_router
from fast_api_app.routes.ripple import docs_router as ripple_docs_router
from fast_api_app.routes.ripple import router as ripple_router
from fast_api_app.routes.ripple_public import admin_router as ripple_admin_router
from fast_api_app.routes.ripple_public import router as ripple_public_router
from fast_api_app.routes.ripple_public import share_router as ripple_share_router
from fast_api_app.routes.search import router as search_router
from fast_api_app.routes.sendou_proxy import router as sendou_proxy_router
from fast_api_app.routes.weapon_info import router as weapon_info_router
from fast_api_app.routes.weapon_leaderboard import (
    router as weapon_leaderboard_router,
)
