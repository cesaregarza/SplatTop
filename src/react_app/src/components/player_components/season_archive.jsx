...
import { getSeasonArchiveRows } from "./playerPageUtils";
...
const SeasonArchive = ({ data, mode, activeSeason, onSeasonChange }) => {
  ...
  const rows = getSeasonArchiveRows(data, mode) || [];
  const currentDisplaySeason = rows.find((row) => row.season_number === activeSeason);
